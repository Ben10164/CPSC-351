...
\begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=2.8cm]
  ...
  \node[state, double]  (3) [below right of=2]  {$q_3$};
...
\begin{center}
    ...
    \begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=2.8cm]
        \tikzstyle{every state}=[fill=none,draw=black,text=black]
        \node[initial,state]    (1)                 {$q_0$};
        \node[state, double]    (2) [right of=1]    {$q_1$};
...
\section{Suppose we have a machine, M4, a DFA, whose alphabet is $\{0,1\}$.  Suppose further that  $L(M_4) = \{w|w$ contains an even number of 0s or contains exactly two 1s$\}$. Give the state diagram for $M_4$}
